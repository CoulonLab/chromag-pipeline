...
def run_cli(imp, channel=0, subpixel=False, multi_time_scale=False, edge_enhance=False,
            virtual=False, only_compute=False, virtual_path=None, verbose=True):
    """Run 3D drift correction in command-line mode
    If you put a ROI, drift will only be computed in this region;
    The ROI will be moved along with the drift to follow your structure of interest.

    Parameters:
    - imp (ImagePlus): 3D stack to drift-correct
    - channel (int): Channel for registration
    - multi_time_scale (bool): Multi_time_scale computation for enhanced detection of slow drifts?
    - subpixel (bool): Sub_pixel drift correction (possibly needed for slow drifts)?
    - edge_enhance (bool): Edge_enhance images for possibly improved drift detection?
    - virtual (bool): Use virtualstack for saving the results to disk to save RAM?
    - only_compute (bool): Only compute drift vectors?
    - virtual_path (str): the path to save the resulting image if using a virtual stack
    - verbose (bool): whether all analysis steps should be logged
    """ 
    ...
    if imp is None:
        return
    if imp.getNFrames() == 1:
        IJ.log("There is only one time frame!")
        return
    
    if virtual is True:
        target_folder = virtual_path
        if not validate(target_folder):
            return
    else:
        target_folder = None 
    ...
    if verbose:
        ...
        IJ.log("    at frame shifts of 1"); 
    dt = 1
    shifts = compute_and_update_frame_translations_dt(imp, channel, dt, edge_enhance)
  
    if multi_time_scale is True: # multi-time-scale computation
        dt_max = imp.getNFrames()-1
        # computing drifts on exponentially increasing time scales 3^i up to 3^6
        # ..one could also do this with 2^i or 4^i
        # ..maybe make this a user choice? did not do this to keep it simple.
        ...
        for dt in dts:
            if dt < dt_max:
                IJ.log("  at frame shifts of "+str(dt)) 
                shifts = compute_and_update_frame_translations_dt(imp, channel, dt, edge_enhance, shifts)
            else: 
                IJ.log("  at frame shifts of "+str(dt_max));
                shifts = compute_and_update_frame_translations_dt(imp, channel, dt_max, edge_enhance, shifts)
                break
    
    shifts = invert_shifts(shifts) # invert measured shifts to make them the correction
    print(shifts)

    registered_imp = None
    if not only_compute: # apply shifts
        IJ.log("    applying shifts..."); print("\nAPPLYING SHIFTS:")
        if subpixel:
            registered_imp = register_hyperstack_subpixel(imp, channel, shifts, target_folder, virtual)
        else:
